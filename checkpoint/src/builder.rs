...
fn s3_cp(local: &Path, dest: &str, endpoint_url: Option<&str>, profile: Option<&str>) -> anyhow::Result<()> {
    ...
    let status = cmd.status().map_err(|e| {
        anyhow::anyhow!(
            "failed to run `aws` CLI: {} — install from https://aws.amazon.com/cli/", e
        )
    })?;
    if !status.success() {
...
fn main() -> anyhow::Result<()> {
    ...
    if let Some(bucket) = &args.upload {
        ...
        s3_cp(&latest_path, &format!("{}/latest.json", bucket), args.endpoint_url.as_deref(), args.profile.as_deref())?;
