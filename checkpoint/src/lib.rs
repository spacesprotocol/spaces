...
pub struct Checkpoint {
    ...
    /// SHA-256 of the checkpoint.tar.gz archive
    pub digest: [u8; 32],
}

/// Remote checkpoint metadata fetched from `latest.json`.
...
pub fn fetch_latest(base_url: &str) -> Result<Option<LatestCheckpoint>, CheckpointError> {
    ...
    let latest: LatestCheckpoint = serde_json::from_str(&body)
...
/// Format the integrity.rs file content with the checkpoint constant.
...
pub const CHECKPOINT: super::Checkpoint = super::Checkpoint {{
    ...
    digest: [{}],
        ...
        height, block_hash, height, digest_str,
    )
}
